...
async function fetchChats(): Promise<ChatRow[]> {
  ...
  try {
    ...
    if (baseUrl) {
      const resAbs = await fetch(`${baseUrl}/api/org/chats`, { cache: "no-store", headers: { cookie } })
      if (resAbs.ok) {
  ...
  const res = await fetch(`${baseUrl}/api/org/chats`, { cache: "no-store", headers: { cookie: '' } })
  if (!res.ok) return []
